...
def dijkstras(G, s):
    ...
    while True: # this will run forever unless there is code below inside the while loop to break the while loop
        d_d = {} # distance dictionary to select which unvisited neighbour to visit next after we calculate the distance to each neighbour from the current node
        ...
        if len(visited) == N: # if we have visited all nodes we have finished the algorithm
            break
        min_val = min(d_d.values()) # we will visit the neighbour with the smallest tentative distance next
